...
public class CompanyArraysRepository implements ICompanyRepository {
    ...
    private final List<Company> myArray; // Cambiado a variable de instancia
    ...
    public boolean save(Company newCompany) {
        if (newCompany == null) {
            return false;  // Manejo seguro de null
        }
    ...
    public Company findByNit(String nit) {
        for (Company company : myArray) {
            if (company.getNit().equals(nit)) {
                return company; // Retorna la compañía encontrada
            }
        }
    ...
    public boolean update(Company company) {
        for (int i = 0; i < myArray.size(); i++) {
            if (myArray.get(i).getNit().equals(company.getNit())) {
                myArray.set(i, company); // Actualiza la compañía
                return true;
            }
        }
